...
# Add togglable sleep somewhere, also Rip out this coordinate translator and 
...
for locations, (lat, lon) in locations.items():
    params = {
        "latitude": lat,
        "longitude": lon,
...
for batch in chunk_list(location_items, BATCH_SIZE):

    names = [item[0] for item in batch]
    lats = [str(item[1][0]) for item in batch]
    lons = [str(item[1][1]) for item in batch]
...
# logic for getting temp to our format of {r,g,b}
...
print("Json From weather constructed succesfully!")
"""

payload = json.dumps(weather_data)
print("Published weather data via MQTT")